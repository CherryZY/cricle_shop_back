...
<div class="tp-admin-nav-container">
	<div class="el-menu-nav">
		...
		<span class="tp-menu-admin">欢迎登陆 <i class="fa fa-user-circle-o" aria-hidden="true"></i>{{accountName}}</span>
		 ...
		 <!-- <span class="el-dropdown-link">
		  ...
		  <!--<el-dropdown-menu slot="dropdown"  class="tp-dropdown-menu">-->
		    ...
		    <!--<el-dropdown-item  divided v-for="item in headedropdownEmall" :key="item.id">-->
		    	...
		    	<!--<p>{{item.message}}<i class="fa fa-times" @click="dropdownClose"></i></p>-->
		    <!--</el-dropdown-item>-->
...
export default{
	...
	data(){
		return {
			...
			headedropdownEmall : [
				{ id:54, addresser : 'Yue',message:'login加入权限登陆，localStorage、state记忆两种登陆方式'},
				{ id:55, addresser : 'Yue',message:'底层采用jquery进行交互，ECharts版本 v3.3.2'},
			],
  ...
  /** 启动页面后进行请求并获取当前页数据 */
    ...
    this.accountName = accountName + " （" + localStorage.getItem('roleName') + "）";
    console.log(token);
	},
      ...
      let _this = this;
			this.$confirm('你确定要退出？')
              ...
              $.ajax({
                ...
                crossDomain: true,
                url: "http://localhost:8899/web/log/out",
                success: function(data){
                  console.log(data);
                  localStorage.removeItem("i-token");
